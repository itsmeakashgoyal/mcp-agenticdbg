...
GROUND_TRUTH: list[CrashGroundTruth] = [
    ...
    CrashGroundTruth(
        ...
        notes=(
            ...
            "the same machine regardless of style. Also ruled out "
            "'demangling_terminate_handler is architecturally unrecoverable': "
            "on a live (pre-save-core) macOS 15.7/older-lldb run, the exact "
            "same chain unwinds all the way through "
            "_objc_terminate -> std::__terminate -> std::terminate -> "
            "__clang_call_terminate -> ScopedTransaction::~ScopedTransaction "
            "(the double-exception thunk) -> process_orders_batch -> main, "
            "and `thread backtrace -e true` (extended backtrace) doesn't "
            "apply here anyway -- that surfaces GCD dispatch-queue enqueue "
            "context, not synchronous C++ unwind info. So this really does "
            "look like an unwind-info regression specific to macOS 26's "
            "lldb/libc++abi build (a very recent OS release), not a "
            "structural limitation TriagePilot's commands could work around; "
            "not fixed here."
        ),
    ),
    CrashGroundTruth(
